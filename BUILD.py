...
def mkCoffee(basefilename):
    coffee = bm.add(
        CoffeeBuildTarget(
            os.path.join("dist", "html", f"{basefilename}.js"),
            [os.path.join("coffee", f"{basefilename}.coffee")],
    ...
    bm.add(
        UglifyJSTarget(
            os.path.join("dist", "html", f"{basefilename}.min.js"),
            coffee.target,
...
bm.add(
    DartSCSSBuildTarget(
        os.path.join("dist", "html", "browser", "aivoice.css"),
        [os.path.join("scss", "style.scss")],
        dependencies=[yarn.target],
        sass_path=SCSS,
    )
)
