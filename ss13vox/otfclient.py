...
class OTFClient:
    def __init__(
        self, address: str, port: int, srvid: str, secret: str
    ) -> None:
    ...
    def calcAuth(self, *samples) -> None:
        ...
        for sample in samples:
            ...
            data += sample
        return hashlib.md5(
            self.session_key.encode("utf-8") + sample
        ).hexdigest()
    ...
    def connect(self) -> None:
        res = self.get("/auth/server", params={"gsid": self.srvid})
        ...
        res = self.post(
            ...
            data={
                ...
                "response": hashlib.md5(
                    (challenge + self.secret).encode("utf-8")
                ).hexdigest(),
            },
        )
        res.raise_for_status()
    ...
    def getSoundFromPhrase(
        self, ckey: str, phrase: str, voice: EVoiceType
    ) -> str:
        data = {
...
def main():
    ...
    client.connect()
    print(
        client.getSoundFromPhrase(
            "testbot", args.phrase, EVoiceType(args.voice)
        )
    )
